...
using namespace std;
using std::size_t;
#define right_turn(a, b, c) ((b.first-a.first)*(c.second-a.second) - (b.second-a.second)*(c.first-a.first) < 0)
...
void OuelletHull::CalcConvexHull() {
	...
	for (size_t i = 0; i != 4; ++i) {
		...
		if (initialization_maximums_for_quadrants[i].first != initialization_maximums_for_quadrants[i].second)
...
vector<pair<double, double>> OuelletHull::GetResultAsVector() {
	...
	for (int i = 1; i != 4; ++i) {
		if (quadrants[i].size() == 1) {
			...
			if (quadrants[i].front() == pointLast)
			...
			quadrant_start_and_end_indexes[i].first = quadrants[i].front() == pointLast ? 1 : 0;
